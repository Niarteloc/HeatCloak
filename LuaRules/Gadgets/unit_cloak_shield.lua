...
local function ValidateCloakShieldDefs(mds)
	...
	for udName, def in pairs(mds) do
		...
		if (not ud) then
			...
			newData.heatCap = def.heatCap or 1000
...
local function UpdateCloakees(data, gameFrame)
    ...
    local heatCap = data.def.heatCap
    for _,cloakee in ipairs(closeUnits) do
		local unitDefID = GetUnitDefID(cloakee)
		...
		if (not ci.scriptReload) and gameFrame and ud.weapons then
			local primaryWeapon = (ci.dyanmicComm and GetUnitRulesParam(cloakee, "primary_weapon_override")) or ci.primaryWeapon
			_, reloaded, reloadFrame = GetUnitWeaponState(cloakee, primaryWeapon)
			if (reloaded == false) then
				local WeaponDefID = ud.weapons[primaryWeapon].weaponDef
				local wd = WeaponDefs[WeaponDefID]
				local reloadTime = Spring.GetUnitWeaponState(cloakee, primaryWeapon, 'reloadTime')
				if (not ci.dyanmicComm) then
					...
					if (reloadFrame > gameFrame + 6) or (GetUnitRulesParam(cloakee, "reloadPaused") ~= 1) then -- UPDATE_PERIOD in unit_attributes.lua.
						reload = ((reloadFrame-gameFrame)/gameSpeed) / ci.reloadTime;
						if (reload >= 0) then

							heatCap = heatCap - reload*wd.customParams.stats_damage
						end
					end
				end
			end
...
--[[		if ci.scriptReload and (ci.scriptReload >= options.minReloadTime.value) then
			...
			if reloadFrame and reloadFrame > gameFrame then
				...
				reload = 1 - (Spring.GetUnitRulesParam(unitID, "scriptReloadPercentage") or (1 - ((reloadFrame - gameFrame)/gameSpeed) / ci.scriptReload))
				if (reload >= 0) then
					heatCap = heatCap - reload*primaryWeapon.def.damage.default
				end
			end
		end]]
    end
	if (heatCap > 0) then
		...
		for _,cloakee in ipairs(closeUnits) do
			SetUnitCloakAndParam(cloakee, false)
			cloakees[cloakee] = true
		end
	end
end
